...
//tran_test
    ...
    testfile<<camera_->getViewMatrix()<< endl;
    testfile<<camera_->getProjectionMatrix()<< endl;
    testfile<<position<< endl;
    testfile<<worldview<< endl;
    testfile<<hcsposition<<endl;
    testfile<<x<< endl;
    testfile<<y<< endl;
    testfile<<z<< endl;
    Ogre::Vector3 test;
    ...
    test.y=1;
    test.z=1;
    Ogre::Vector3 test_trans=camera_->getProjectionMatrix() * test;
    testfile<<test<<endl;
    testfile<<test_trans<<endl;
    Ogre::Vector3 test_1;
    test_1.x=1;
    test_1.y=2;
    test_1.z=3;
    Ogre::Vector3 test_trans_1=camera_->getProjectionMatrix() * test_1;
    testfile<<camera_->getProjectionMatrix()<<endl;
    testfile<<test_1<<endl;
    testfile<<test_trans_1<<endl;
    testfile.close();
    }
  }
...
// 	(double)pos_world.x,(double)pos_world.y,(double)pos_world.z);
    jointPositionMap[name] = cv::Vec6d(
      (double)screen_x,(double)screen_y,(double)screen_z,pos_world[0],pos_world[1],pos_world[2]);    
  }
  
  void walk_bones(HandRenderer::JointPositionMap&jointPositionMap,Node* bone = NULL,int depth = 0)
  {//tran_test
    bool tran_test=false;
    ...
    get2dposition(pos_world, screen_x,screen_y,screen_z,tran_test);
    tran_test=false;
	...
	.yAxis().normalisedCopy();
      ...
      get2dposition(tip_pos, screen_x,screen_y,screen_z,tran_test);
...
//lx_test
float  HandRenderer::hand_camera_distance(){
 return private_->CameraHandDistance();
}
void HandRenderer::get_camera_info(){
private_->GetCameraInfo();
}
...
void HandRendererPrivate::GetCameraInfo(){
   ...
   {
     for(int j=0;j<4;j++)
     {camerafile<<camera_->getViewMatrix()[i][j]<<",";}
   }
   for(int i =0; i<4;i++)
   {
     for(int j=0;j<4;j++)
     {camerafile<<camera_->getProjectionMatrix()[i][j]<<",";}
   }
   camerafile<<0;
   camerafile.close();
}
